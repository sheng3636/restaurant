  ...
  <div class="appContainer">
    <div class="mainWrapper">
      <div class="mainHeader">
        <span class="title">模板查询</span>
        <el-form class="searchBar" ref="queryParams" :model="queryParams" label-width="80px" label-position="top">
          <el-form-item label="" prop="library">
            <el-input v-model="queryParams.library" placeholder="请输入数据库名称" />
          </el-form-item>
          <el-form-item label="" prop="pathPrefix">
            <el-input v-model="queryParams.pathPrefix" placeholder="请输入生成目录" />
          </el-form-item>
          <el-form-item label="" prop="pathSuffix">
            <el-input v-model="queryParams.pathSuffix" placeholder="请输入代码包" />
          </el-form-item>
          <div class="btnGroup">
    ...
    <div class="mainWrapper"><div class="mainHeader mainHeader1">
        <span class="title">模板列表</span>
        <div class="searchBar">
          <el-button type="primary" @click="addTemplateFn()"><i class="el-icon-plus"></i>新增</el-button>
        </div>
      </div>
    ...
    <el-dialog :title="title" :visible.sync="popup" :close-on-click-modal="false" :close-on-press-escape="false"
      :before-close="dialogClose" width="50%">
      <el-form ref="popupForm" class="dialogBody" :rules="AddEditRules" :model="form" label-width="80px"
        label-position="top">
        <el-row>
          <el-col :span="12">
            <el-form-item label="项目名" prop="projectName">
        ...
        <el-collapse v-model="activeNames" @change="handleChange">
          <el-collapse-item title="生成java代码" name="1">
            <el-transfer filterable :filter-method="filterMethod" filter-placeholder="请输入表名" v-model="form.value"
              :render-content="renderFunc" :props="{
                key: 'tableName',
                label: 'tableComment'
          ...
          <el-collapse-item title="生成vue代码:筛选项" name="2">
            <el-transfer filterable :filter-method="filterMethod1" filter-placeholder="请输入列名" v-model="form.filters"
              :render-content="renderFunc1" :titles="filtersTitle" :props="{
                key: 'columnName',
                label: 'columnComment'
              }" :data="filters" />
          </el-collapse-item>
          <el-collapse-item title="生成vue代码:显示列" name="3">
            <el-transfer filterable :filter-method="filterMethod1" filter-placeholder="请输入列名" v-model="form.columns"
              :titles="columnsTitle" :render-content="renderFunc1" :props="{
                key: 'columnName',
                label: 'columnComment'
              }" :data="columns" />
          </el-collapse-item>
          <el-collapse-item title="生成vue代码:编辑列" name="4">
            <el-transfer filterable :filter-method="filterMethod1" filter-placeholder="请输入列名" :titles="editColumnsTitle"
              v-model="form.editColumns" :render-content="renderFunc1" :props="{
                key: 'columnName',
                label: 'columnComment'
              }" :data="editColumns" />
          </el-collapse-item>
...
export default {
  data() {
    return {
      ...
      filterMethod(query, item) {
        return (
          item.tableName.indexOf(query) > -1 ||
          item.tableComment.indexOf(query) > -1
        )
      },
      filterMethod1(query, item) {
        return (
          item.columnName.indexOf(query) > -1 ||
          item.columnComment.indexOf(query) > -1
        )
      },
      renderFunc(h, option) {
        return (
          <span>
            {option.tableName} - {option.tableComment}
          </span>
        )
      },
      renderFunc1(h, option) {
        return (
          <span>
            {option.columnName} - {option.columnComment}
          </span>
        )
      },
  ...
  methods: {
    ...
    generateFn(row) {
      ...
      apiGet(this, this.generateCode, { tableSchema: row.library }).then(
        (res) => {
          this.data = res.data
